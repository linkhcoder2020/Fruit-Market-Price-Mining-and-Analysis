...
orange_url = "https://www.marketnews.usda.gov/mnp/fv-report-top-filters?" \
             ...
             "shipNavClass=&movNavClass=&stateID=&volume=&repType=termPriceDaily&locAbr=AJ&" \
...
limes_url = "https://www.marketnews.usda.gov/mnp/fv-report-top-filters?" \
            ...
            "shipNavClass=&movNavClass=&stateID=&volume=&repType=termPriceDaily&locAbr=BP&" \
            "environment=&varName=SEEDED+TYPES&organic=&repDate={date}Go=Go"
...
guava_url = "https://www.marketnews.usda.gov/mnp/fv-report-top-filters?" \
            ...
            "movNavClass=&stateID=&volume=&repType=termPriceDaily&locAbr=HX&environment=&varName=" \
            "&organic=&repDate={date}&Go=Go"

strawberries_url = "https://www.marketnews.usda.gov/mnp/fv-report-top-filters?" \
                   "commAbr=STRBY&repDate={date}&varName=&locAbr=AJ&repType=termPriceDaily&" \
...
blueberries_url = "https://www.marketnews.usda.gov/mnp/fv-report-top-filters?" \
                  "commAbr=BLUBY&repDate={date}&locAbr=AJ&repType=termPriceDaily&" \
...
# 水果名稱對應的url (Url corresponding to fruit name)
KIND_MAP_URL = {
    "ORANGES": orange_url,
    "LIMES": limes_url,
    "GUAVA": guava_url,
...
# 開始日期(start date)，2019，1，1號會查找失败(2019, 1, 1 will fail)。 使用者要先上去網站上檢查哪個日期可以使用，在這裡輸入正確日期，否則得到的資料將會是當日資料
START_DATE = (2019, 1, 2)
# 结束日期(end date) 2019，12，31
END_DATE = (2019, 12, 31)
# csv文件頭 (csv header)
HEADER = [
    "Commodity_Name", "City", "Date", "Low_Price", "High_Price", "Origin", 'Item_Size'
]
