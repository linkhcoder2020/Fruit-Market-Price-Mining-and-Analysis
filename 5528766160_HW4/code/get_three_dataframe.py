...
from config import KIND_MAP_URL, LOCATIONS
...
# 抽取頁數的正則表達式 (Regular expression for extracting pages)
pat_page = re.compile(
...
def get_ua():
    ...
    ua = UserAgent(path=location,verify_ssl=False)  #verify_ssl=False
    return ua.random
...
def get_and_format_data(url, all_data, kind, location, everyday):
    ...
    for pat_obj in re.finditer(pat_row, html):
        # 正则獲取每一行數據 (get each row of data by using re)
        data_row = [
            kind,
            location,
            everyday,
...
def main_task(filepath='./fruit_price.csv'):
    """
    ...
    for kind in KIND_MAP_URL:
        ...
        for location in LOCATIONS.get(kind):
            # each day on 2019/01/02-2019/12/31
            for everyday in date_list:
                ...
                url = base_url.format(date=everyday.replace('/', '%2F'), location=location.replace(' ', '+'))
...
if __name__ == '__main__':
    ...
    main_task('./fruit_price.csv')
    fruit_csvdata = pd.read_csv('fruit_price.csv')
    fruit_price_df = pd.DataFrame(fruit_csvdata)

    print("------ Start to get place and weather data -----")
    place_df, weather_df= get_weather_place_data()
